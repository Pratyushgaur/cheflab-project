...
class UserControllers extends Controller
{
    ...
    public function chef_edit($encrypt_id)
    {
        ...
        try {
            ...
            //dd($vendor);die;
            $categories = @Catogory_master::where('is_active', '=', '1')->pluck('name', 'id')->toArray();
            ; //->get();
    ...
    function create_system_user(Request $request)
    {
        $roles = \App\Models\Roles::get();
        return view('admin.system_user.create',compact('roles'));
    }

    public function checkSystemUserExist(Request $request)
    {
        if (\App\Models\UserAdmin::where('email', '=', $request->email)->exists()) {
            return \Response::json(false);
        } else {
            return \Response::json(true);
        }
    }
    function post_system_user(Request $request) {
        $this->validate($request, [
            'user_name' => 'required',
            'email' => 'required',
            'phone' => 'required',
        ...
        $admin = new \App\Models\UserAdmin;
